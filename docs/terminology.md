...
[MVC介绍](https://baike.baidu.com/item/MVC%E6%A1%86%E6%9E%B6/9241230?fr=aladdin&fromid=85990&fromtitle=MVC)

## 技术选型

`参考`

[关于Web大型系统的技术选型](http://blog.csdn.net/jacky4955/article/details/44979235)

[前端架构技术选型？](https://www.zhihu.com/question/41316607)

[我们是如何进行技术选型的？](http://www.360doc.com/content/16/0426/08/32626470_553847410.shtml)

[漫谈技术选型](http://blog.csdn.net/phospher/article/details/51546042)
...
在计算机身份认证中是令牌（临时）的意思，在词法分析中是标记的意思。在Web领域基于Token的身份验证随处可见。在大多数使用Web API的互联网公司中，tokens 是多用户下处理认证的最佳方式。
...
我们都是知道HTTP协议是无状态的，这种无状态意味着程序需要验证每一次请求，从而辨别客户端的身份。在这之前，程序都是通过在服务端存储的登录信息来辨别请求的。这种方式一般都是通过存储Session来完成。但随着Web，应用程序，以及移动端的兴起，这种验证的方式逐渐暴露出了问题。尤其是在可扩展性方面。
...
[关于APP token验证的疑问?](https://www.zhihu.com/question/30267006)
...
我的理解是，前端和后端从以前的高耦合合作变为低耦合甚至为0耦合合作，前端和后端开发工程师各做各的，各用各的语言框架，由串行，变为并行，两端通过nodejs（中间层），利用api接口进行数据的传输。

也就意味着，前后端之间使用 JSON 来交流，两个开发团队之间使用 API 作为契约进行交互。从此，后台选用的技术栈不影响前台。当后台开发人员选择 Java 的时候，我可以不用 JSP 来编写前端页面，继续使用我的 React 又或者 Angular。而我使用 React 时，也不影响后台使用某一个框架。

`参考`

[淘宝前后端分离实践](http://2014.jsconf.cn/slides/herman-taobaoweb/index.html#/100)
...
[hadoop-百度百科](https://baike.baidu.com/item/Hadoop/3526507?fr=aladdin)
